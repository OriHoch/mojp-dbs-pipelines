...
@pytest.mark.skip("test makes an http call, kept for documentation / future development")
def test_invalid_call():
    try:
        ClearmashApi()._wcm_api_call("/invalid/path")
        assert False
...
def test_get_web_document_system_folder_places():
    ...
    assert len(items) == 10
    item = items[9]
    assert item["Id"] == 115470
    assert item["Name"] == 'ראדומסק , ראדומסקו'
...
def test_get_documents_places():
    ...
    assert first_entity.pop("Changeset") == 6477739
