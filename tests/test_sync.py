...
from.bagnowka.test_convert import get_bagnowka_convert_resource_data
...
def assert_sync_processor(input_data=None):
    input_data = get_bagnowka_convert_resource_data()
    processor = CommonSyncProcessor(parameters={"input-resource": "dbs-docs", "output-resource": "dbs-docs-sync-log"},
                                    datapackage={"resources": [{"name": "dbs-docs"}]},
                                    resources=[input_data],
                                    settings=get_mock_settings())
    return assert_processor(processor)
    assert input_data == ""
